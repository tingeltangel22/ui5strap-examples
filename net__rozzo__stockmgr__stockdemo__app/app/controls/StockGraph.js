 ...
 */
 ...
 (function(){
 	...
 	jQuery.sap.declare("net__rozzo__stockmgr.stockdemo__app.controls.StockGraph");
 	jQuery.sap.require("ui5strap.library");

	ui5strap.Control.extend("net__rozzo__stockmgr.stockdemo__app.controls.StockGraph", {
 		metadata : {
 	...
 	var StockGraphProto = net__rozzo__stockmgr.stockdemo__app.controls.StockGraph.prototype;
 	...
 	StockGraphProto.clear = function(){
 		...
 		graphContainer.append('div').attr('class', 'please-wait').text('Please wait...');
...
graph = graphSvg.append("svg:g").attr('class', 'root-container').attr("transform", "translate(" + m[3] + "," + m[0] + ")");      


var data = this.prepareData(); console.log(data);

container.selectAll('.please-wait').remove();

if(false === data){
	container.append('div').attr('class', 'please-wait').text('No Data');
	return false;
}
...
var barWidth = 16;
	...
	var x2 = d3.time.scale().domain([data.minDate, data.maxDate]).range([0, w]);

	// Y scale will fit values from 0-10 within pixels h-0 (Note the inverted domain for the y-scale: bigger is up!)
	//var y = d3.scale.linear().domain([0, 10]).range([h, 0]);
		// automatically determining max range can work something like this
		var space = (data.maxAmount - data.minAmount) / 10;
		var y = d3.scale.linear().domain([Math.min(0, data.minAmount - space), data.maxAmount+space]).range([h, 0]);
...
.ticks(d3.time.day, 1).tickFormat(function(d, i){
    ...
    return d.getDate(); //"Year1 Year2, etc depending on the tick value - 0,1,2,3,4"
})
*/
var color = d3.scale.category20();
		...
		var bar = graph.selectAll("g.stock_overview-graph-bar")
		.data(data.players);

		bar.enter()
		.append("svg:g")
		.attr("class", "stock_overview-graph-bar");
		...
		barSegment.transition().delay(250)
		.attr("y", function(d, i){
			return y(d.start) - (d.value > 0 ? y(0) - y(Math.abs(d.value)) : 0);
		})
		.attr("height", function(d, i){
			return y(0) - y(Math.abs(d.value)) - 1.5;
		});

		var stockLineContainer = graph.append("svg:g").attr("transform", "translate(0,0)").attr('width', 0);

		var stockLine = stockLineContainer.append("svg:path")

		.attr('class', 'stock_overview-graph-stock-line')
		.attr("stroke", function(d,i){ return color(i); });


		stockLine.attr("d", line(data.players));

		stockLineContainer.selectAll('circle.stock_overview-graph-stock-line-circle')
		.data(data.players)
		.enter()
		.append('svg:circle')
		.attr('class', function(d, i){
			return 'stock_overview-graph-stock-line-circle stock_overview-graph-stock-line-circle-' + (d.stock >= data.safetyStock ? 'beyond' : 'below');
		})
		...
		.attr('x1', -barWidth)
		.attr('y1', y(data.safetyStock))

		.attr('y2', y(data.safetyStock))
		.attr('x2', -barWidth)
		...
		//graph.append("svg:path").attr("d", line(dataSpent)).attr('class', 'spent');
		//graph.append("svg:path").attr("d", line(dataTotal)).attr('class', 'total');
	};
